...
uint8_t pgm_read_byte(const uint8_t *b)
{
    return *b;
}
...
struct mSerial
{
    ...
    void print(int val, uint8_t base = DEC)
    {
        //cout << string;
        QByteArray res = QByteArray::number(val,base);
        cout << res;
    }
...
{
    qDebug() << "D:digitalWrite("<< port << ", " << state << ")";
    if(port == 5)
    {
        //mSpiClass->setIOLowBit(csPin,state?FT_IO_SET:FT_IO_RESET);
    }
}
void pinMode(uint8_t port, uint8_t state)
{
    if(port != 5)
        return;
    mSpiClass = new FtSpi();
    mSpiClass->setUsbSetting(0x0403,0x6010,1);
    if (!mSpiClass->usbInit())
    {
        qDebug() << "Error initing!"<< mSpiClass->getError();
        qErrnoWarning("Error!");
    }

    mSpiClass->setFrequency(500000);

    uint8_t direction = mSpiClass->getIOLowDirection() & ~(0b111);
    direction |= (0b011 | (1 <<csPin));
    mSpiClass->setIOLowDirection(direction);

    mSpiClass->options = 0x01;
    mSpiClass->setIOLowBit(0,FT_IO_RESET);

    qDebug() << "D:pinMode("<< port << ", " << state << ")";
}
uint8_t digitalRead(uint8_t pin)
{
    qDebug() << "D:digitalRead("<< pin << ")";
    return 1;
}
...
struct mSPI
{
    ...
    {
        //mSpiClass
        qDebug() << "D:setBitOrder("<< order << ")";
    }
    void setDataMode(uint8_t mode)
    {
        qDebug() << "D:setDataMode("<< mode << ")";
    }
    uint8_t mulTransfer(uint8_t *data, uint8_t size, uint8_t *ack = 0)
    {
        //qDebug()<<"mulTransfer("<<QByteArray((char*)data,size).toHex()<<", "<<size<<", " << (ack ? "ACK)":"0)");
        mSpiClass->setIOLowBit(csPin,FT_IO_RESET);
        mSpiClass->options |= 0x30;
        //uint8_t answer = data;
        uint8_t buf[255];
        memcpy(buf,data,size);
        mSpiClass->readWrite(buf,size);
        mSpiClass->setIOLowBit(csPin,FT_IO_SET);
        if(ack)
            memcpy(ack,buf,size);
        return size;
...
/*	digitalWrite(_chipSelectPin, LOW);		// Select slave
    ...
    for(uint8_t i=0;i<count; i++)
        buf[i] = address;
    buf[count] = 0;
    SPI.mulTransfer(buf,count+1,ack);
    if(rxAlign)
    {		// Only update bit positions rxAlign..7 in values[0]
        ...
        // Apply mask to both current value of values[0] and the new data in value.
        values[0] = (values[0] & ~mask) | (ack[1] & mask);
        for(uint8_t i=2;i<count +1 ; i++)
            values[i-1] = ack[i];
    }
    else
    {
